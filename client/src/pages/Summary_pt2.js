...
const Summary = ()=>{
    ...
    const {Nutrition, Fitness} = results;
    const style_div = { 
        paddingTop: '1cm',
        paddingLeft: '0.5cm',
        color: "#1A1A1B",
    ...
    return( 
        <div style={style_div} className="SummaryPt2 PhenotypeReport">
            ...
            {loading ? (
                ...
                <div> 
                    <TableSummary panel_name={'Nutrition'} details = {Nutrition}/> 
                </div>
    ...
    return(
        ...
        <div className="d-flex flex-column justify-content align-items-center" style={style_table}>
                <div style={{width: "100%",paddingLeft: "5px" ,padding: "1px" ,marginBottom: "0cm", backgroundColor: "", color: "#2c3131", borderRadius: ""}}>
                    <h1 style={{borderBottom: "2px solid #2c3131"}}>{panel_name}</h1>
                </div>
                <div>
                    {spliced.map(
                        x=><div className="d-flex flex-row" style={style_row}>{x.map(
